...
```{r}
mydb <- dbConnect(RSQLite::SQLite(), "ngrams_db4.sqlite")
query <- paste0("SELECT * FROM n_grams3")
result_df <- dbGetQuery(mydb, query)
# Building freq table
result_df <- result_df %>% group_by(word1,word2,word3) %>% mutate(prob_n = n / sum(n)) %>% arrange(desc(word1,word2)) %>% select(word1,word2,word3,prob_n)
dbDisconnect(mydb)
# Building freq table database
mydb <- dbConnect(RSQLite::SQLite(), "ngrams_db_prob.sqlite")
...
```{r}
mydb <- dbConnect(RSQLite::SQLite(), "ngrams_db4.sqlite")
query <- paste0("SELECT * FROM n_grams2")
result_df <- dbGetQuery(mydb, query)
# Building freq table
result_df <- result_df %>% group_by(word1,word2) %>% mutate(prob_n = n / sum(n)) %>% arrange(desc(word1)) %>% select(word1,word2,prob_n)
dbDisconnect(mydb)
# Building freq table database
mydb <- dbConnect(RSQLite::SQLite(), "ngrams_db_prob.sqlite")
