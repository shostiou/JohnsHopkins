...
The approach taken to answer our 2 initial questions will be to build a linear model with the mpg variable as a response variable and a set of regressors as inputs.  
This model will provide us elements of interpretation to understand how the different regressors kept in our model can influence the miles per gallon concern.\
...
In this step, we will fit a set of models by conducting a forward approach (step by step approach with new variables added at each step). Note that as stated above thanks to the correlation analysis, disp & cyl variables will not be embedded into our model. Considering, moderate correlation values between our response variable and qsec, gear and carb, we will not consider those variables as well.\
mpg will be our response variable and as our questions are related to the impact of transmission mode, the am variable will be embedded in each instance of the model. Our categorical variables will be dealt as factors. Anova will be applied to select the best model (nested likelihood ratio tests).\
...
anova(fit0,fit5)
```
Here, we control the evolution of the coefficient associated to the "am" variable.
...
The coefficients of the model can be interpreted as follow :\
...
```{r}
ggplot(my_mtcars)+
  geom_boxplot(mapping = aes (x=am,y=mpg, fill = am))+
  ggtitle("Miles par gallon per Transmission 0 = auto / 1 = man")
```

```{r}
ggplot(my_mtcars)+
  geom_histogram(mapping = aes (mpg, fill = am),bins=12)+
  ggtitle("Miles par gallon per Transmission 0 = auto / 1 = man")
```
